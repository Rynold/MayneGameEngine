...
void Mesh::DrawPlane(Shader* shader, GLuint shadowMap)
{
	...
	glUniform1f(glGetUniformLocation(shader->Program, "material.roughness"), material->roughness);
...
void Mesh::DrawMesh(Shader* shader, GLuint shadowMap)
{
	if (type == CUBE)
		DrawCube(shader, shadowMap);
	else if (type == PLANE)
		DrawPlane(shader, shadowMap);
}

void Mesh::DrawCube(Shader* shader, GLuint shadowMap)
{
	if (material->baseTexture != NULL){
		glUniform1i(glGetUniformLocation(shader->Program, "material.baseTex"), 0);
		glActiveTexture(GL_TEXTURE0);
	...
	if (material->roughnessMap != NULL){
		glUniform1i(glGetUniformLocation(shader->Program, "material.roughnessMap"), 4);
		glActiveTexture(GL_TEXTURE4);
		glBindTexture(GL_TEXTURE_2D, material->roughnessMap);
	}

	if (material->heightMap != NULL){
		glUniform1i(glGetUniformLocation(shader->Program, "material.depthMap"), 5);
